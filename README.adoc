...
:link-hmac: http://en.wikipedia.org/wiki/HMAC[HMAC]
...
:link-damontools: http://cr.yp.to/daemontools.html[daemontools]
:link-runit: http://smarden.org/runit/[runit]
:link-upstart: http://upstart.ubuntu.com/[upstart]
:link-launchd: http://launchd.macosforge.org/[launchd]
:link-systemd: https://www.freedesktop.org/wiki/Software/systemd/[systemd]
:link-daemontools-why: http://cr.yp.to/daemontools/faq/create.html#why[daemontools/why]
...
:link-releases: http://github.com/cactus/go-camo/releases[binary releases]
:link-mit-license: http://www.opensource.org/licenses/mit-license.php[MIT license]
...
A camo server is a special type of image proxy that proxies non-secure images over SSL/TLS, in order to prevent mixed content warnings on secure pages.
The server works in conjunction with back-end code that rewrites image URLs and signs them with an {link-hmac}.
...
While Go-Camo will support proxying HTTPS images as well, for performance reasons you may choose to filter HTTPS requests out from proxying, and let the client simply fetch those as they are.
...
*   Go-Camo allows custom default headers to be added -- useful for things like adding {link-hsts} headers.
...
Please note, however, that this does not provide protecton for a network that uses public address space (ipv4 or ipv6), or some of the {link-ip6-special-addresses}[more exotic] ipv6 addresses.
...
+
--
If a `filter-ruleset` file is defined, that file is read and each line converted into a filter rule.
See link:man/go-camo-filtering.5.adoc[`go-camo-filtering(5)`] for more information regarding the format for the filter file itself.

Regarding evaluatation: The ruleset is NOT evaluated in-order.
The rules process in two phases: "allow rule phase" where the allow rules are evaluated, and the "deny rule phase" where the deny rules are evaluated.
...
The `filter-ruleset` functionality (both allow and deny) is supplied predominantly as a fallback safety measure for cases where you have previously generated a url and you need a quick temporary fix, or for cases where rolling keys takes a while and/or is difficult.
====
--
...
+
--
The `--max-size` value is defined in KB. Set to `0` to disable size restriction.
The default is `0`.
--
...
When the `--metrics` flag is used, the service will expose a Prometheus-compatible `/metrics` endpoint.
...
These containers are untested and provided only for those with specific containerization requirements.
When in doubt, prefer the statically compiled {link-releases}, unless you specifically need a container.

== Alternative Implementations

*   {link-mrsaints}' go-camo {link-arachnys-fork} -- supports proxying additional content types (fonts/css).
