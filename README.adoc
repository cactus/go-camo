...
Available recipes:
    ...
    build-oci-image *FLAGS  # build container images
    clean                   # clean
    cross-build             # build cross compiled binaries
    manpages                # build man pages

    [checks]
    check                   # run all ~checks:default checkers
    check-optional          # run all ~checks:optional checkers

    [hygiene]
    modernize               # run modernize
    update-go-deps          # update go.mod dependencies

    [release]
    cross-tar               # build cross compiled tarballs
    release                 # build binaries tarball
    release-sign            # build binaries tarball
    tar                     # build binaries tarball

    [tests]
    bench +FLAGS='-vet=off' # run benchmarks
    cover                   # run coverage analysis
    test                    # run tests

    [~checks:default]
    betteralign             # run betteralign
    ...
    [~checks:optional]
    deadcode                # run deadcode (will have false positives)
    errorlint               # run errorlint (will have false positives)
    nilaway                 # run nilaway (will have false positives)
